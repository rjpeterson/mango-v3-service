...
class MangoSimpleClient {
    constructor(connection, client, mangoGroupConfig, mangoGroup, owner, mangoAccount) {
    ...
    static async create() {
        ...
        const key = JSON.parse(fs_1.default.readFileSync(privateKeyPath, "utf-8"));
        const owner = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(key));
        let mangoAccount;
        if (process.env.MANGO_ACCOUNT) {
            ...
            utils_1.logger.info(`owner info: ${JSON.stringify(owner)}`);
        ...
        return new MangoSimpleClient(connection, mangoClient, mangoGroupConfig, mangoGroup, owner, mangoAccount);
    }
    /// public
